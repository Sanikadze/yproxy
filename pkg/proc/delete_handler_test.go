...
func TestListDelete2Files(t *testing.T) {
	...
	storage := mock.NewMockStorageInteractor(ctrl)
	storage.EXPECT().ListBucketPath(gomock.Any(), "", msg.Prefix, true).Return(filesInStorage, nil)
	...
	actualFilesToDelete, err := handler.ListDelete2Files(context.Background(), "", msg)
	assert.NoError(t, err)
