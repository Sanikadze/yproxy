	...
	for len(fileList) > 0 && retryCount < 10 {
		...
		for i := 0; i < len(fileList); i++ {
			...
			if !msg.Garbage {
				...
				deleteErr = dh.StorageInterractor.DeleteObject(ctx, bucket, fileList[i].Path)

			}
			if deleteErr != nil {
				ylogger.Zero.Warn().AnErr("err", deleteErr).Str("bucket", bucket).Str("file", fileList[i].Path).Msg("failed to delete file")
				failed = append(failed, fileList[i])
				err = deleteErr
			}
		}
