...
func (sp *S3SessionPool) createSession(cr *config.StorageCredentials) (*session.Session, error) {
	...
	if sp.cnf.S3DialTimeout > 0 || sp.cnf.S3ResponseHeaderTimeout > 0 || sp.cnf.S3IdleConnTimeout > 0 {
		transport := &http.Transport{
			DialContext:           (&net.Dialer{Timeout: time.Duration(sp.cnf.S3DialTimeout), KeepAlive: 15 * time.Second}).DialContext,
			TLSHandshakeTimeout:  time.Duration(sp.cnf.S3DialTimeout),
			ResponseHeaderTimeout: time.Duration(sp.cnf.S3ResponseHeaderTimeout),
			IdleConnTimeout:       time.Duration(sp.cnf.S3IdleConnTimeout),
